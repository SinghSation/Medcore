...
/**
 * Handler for [AddProblemCommand] (Phase 4E.2).
 *
 * Runs inside the [com.medcore.platform.write.WriteGate]-owned
 * transaction after [AddProblemPolicy] has approved
 * `PROBLEM_WRITE`. `PhiRlsTxHook` has set both RLS GUCs by
 ...
 * 4. INSERT a new problem row with `status = ACTIVE`,
 *    `tenant_id` and `patient_id` from the verified parents.
 *    `saveAndFlush` so `row_version` is populated before the
 *    auditor runs.
 * 5. Return [ProblemSnapshot].
 *
 * ### No-op suppression
 *
 * Add is never a no-op — every call mints a new UUID and a new
 * row. Sibling problems (same condition text, same patient,
 * different rows) are intentionally allowed in 4E.2: free-text
 * dedup is unreliable, coded-condition dedup arrives with 5A
 * FHIR + 3M reference data. Clinicians who genuinely double-
...
class AddProblemHandler(
    ...
    fun handle(
        ...
        val entity = ProblemEntity(
            ...
            abatementDate = command.abatementDate,
        ...
        val saved = problemRepository.saveAndFlush(entity)
        return ProblemSnapshot.from(saved)
    }
}
