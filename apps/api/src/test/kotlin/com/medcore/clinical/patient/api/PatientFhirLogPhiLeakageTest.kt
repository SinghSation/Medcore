...
/**
 * Rule 01 operational control for the FHIR Patient read path
 * (Phase 4A.5). Extends the 4A.4 native-GET leakage test with
 * FHIR-shape coverage.
 *
 * The FHIR endpoint returns a richer structure (identifier
 * system URI, extensions, communication) — more fields, same
 * no-leak discipline. Distinctive PHI tokens go in, stdout
 * comes back clean.
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@Import(TestcontainersConfiguration::class)
@ExtendWith(OutputCaptureExtension::class)
class PatientFhirLogPhiLeakageTest {
    ...
    @Test
    fun `patient name and demographics never appear in logs on FHIR GET`(
        output: CapturedOutput,
    ) {
        val subject = "phi-fhir-${UUID.randomUUID()}"
        val token = tokenFor(subject)

        val distinctiveGiven = "Zyxwvut-FhirGiven-${UUID.randomUUID()}"
        val distinctiveFamily = "Qponmlk-FhirFamily-${UUID.randomUUID()}"
        ...
        // Seed the patient via the native POST path (write).
        val createBody = """
        ...
        // Read via the FHIR path — second place a log leak could
        // happen beyond the 4A.4-covered native path.
        val response = fhirGet(token, "acme-health", patientId)
        assertThat(response.statusCode).isEqualTo(HttpStatus.OK)

        val captured = output.all
        assertThat(captured)
            .describedAs("patient nameGiven must never appear in logs on FHIR GET")
            .doesNotContain(distinctiveGiven)
        assertThat(captured)
            .describedAs("patient nameFamily must never appear in logs on FHIR GET")
            .doesNotContain(distinctiveFamily)
        assertThat(captured)
            .describedAs("patient preferredLanguage must never appear in logs on FHIR GET")
